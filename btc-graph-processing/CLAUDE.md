...
7. **Имена состояний выводятся из статистики, а не задаются руками.**
...
8. **Порог дробления состояний относительный, а не абсолютный.** Эффективное
...
| Добавить тип события | `btcproc/features/events.py` (`ATOM_FAMILY` + `detect_atoms`, только в конец словаря) |
