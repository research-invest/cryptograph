...
def detect_atoms(base: pd.DataFrame) -> pd.DataFrame:
    ...
    #
    # ВЫРОЖДЕН ДЛЯ ДЕШЁВЫХ МОНЕТ. Порог здесь в долларах, а не в долях цены —
    # единственное такое место в проекте (см. правило «ни один порог не в
    # долларах», SMCConfig в config.py). На SOL в диапазоне $100–200 условие
    # `close % 1000 < 50` не выполняется никогда: атом мёртв, бит 19 маски
    # всегда нулевой. На BTC он работает по замыслу, на ETH — вырожденно
    # (срабатывает на широких полосах, а не на круглых уровнях).
    #
    # Формулу НЕ править на месте: это signature-атом, бит 19 маски, и смена
    # семантики переопределяет смысл всех исторических event_block_id без
    # единой ошибки. Рабочий детектор заводится только новым атомом В КОНЕЦ
    # ATOM_FAMILY (масштабный шаг 10^floor(log10(close)), близость в долях
    # шага), сначала как контекстный — лифт по нему меряется даром
    # (scripts/measure_atom_lift.py), — и лишь потом обсуждается перевод
    # в signature. Порядок — docs/extending_features.md.
