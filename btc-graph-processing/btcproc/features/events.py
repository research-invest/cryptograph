...
# Версия НАБОРА атомов, собранная из состава включённых источников:
# "v1", "v1+smc", "v1+onchain+smc". По той же схеме, что feature_version()
# в builder.py, и по той же причине.
#
# До 2026-08-11 константа была объявлена и нигде не использовалась. Это не
# мелочь: event_block_id — хэш битовой маски, и смысл блока задан не только
# маской, но и тем, какие детекторы её считали. Два прогона с разным набором
# атомов пишут в bar_events строки, внешне неотличимые: те же колонки, те же
# идентификаторы блоков, правдоподобные значения. Сейчас, по рамке «данные
# можно пересобрать», это ничем не грозит — набор атомов меняется свободно, —
# но к началу эксплуатации механизм должен уже стоять: вводить его
# ретроспективно означало бы разбирать, каким набором размечена каждая строка.
BASE_EVENT_VERSION = "v1"


def event_version() -> str:
    """
    Метка текущего набора атомов.

    Зависит от флага АТОМОВ источника, а не признаков: включённые признаки
    состав атомов не меняют, и наоборот.
    """
    from btcproc.features import registry

    return registry.compose_version(
        BASE_EVENT_VERSION, registry.enabled_atom_sources()
    )
...
#: Близость к круглому уровню — в долях масштабного шага. 0.05 с каждой
#: стороны даёт 10% баров: столько же, сколько давала прежняя формула
#: `close % 1000 < 50` на биткоине его основной эпохи, поэтому гранулярность
#: блоков событий от смены детектора не поехала.
ROUND_LEVEL_TOLERANCE = 0.05


def round_level_touch(close: pd.Series) -> pd.Series:
    """
    Цена у круглого уровня: тысячи долларов притягивают заявки и стопы.

    Шаг уровня масштабируется по порядку величины цены:

        шаг = 10^(⌊log₁₀(цена)⌋ − 1)

    BTC за $60 000 → шаг $1000, ETH за $3000 → $100, SOL за $150 → $10. Для
    биткоина его основной эпохи это в точности прежний детектор
    (`close % 1000 < 50`), для остальных монет — его смысл, а не его число.

    Почему формулу пришлось менять. Порог был задан В ДОЛЛАРАХ — единственное
    такое место в проекте (правило «ни один порог не в долларах» — см.
    `SMCConfig`). На SOL в диапазоне $100–200 условие `close % 1000 < 50` не
    выполняется никогда: атом мёртв, бит 19 маски у монеты постоянно нулевой.
    На ETH он срабатывал вырожденно — на широких полосах, а не на круглых
    уровнях. То есть signature-атом, за который платили дроблением блоков,
    работал ровно на одной монете из трёх.

    Смена семантики переопределяет смысл всех исторических `event_block_id` —
    это осознанная плата: по рамке «накопленные данные можно пересобрать»
    (задача 11-101, §0.1) правка идёт в общий ломающий пакет, после которого
    граф переобучается целиком. Пока эта рамка не действовала, правку на
    месте делать было нельзя, и в коде стоял запрет.

    Масштабная инвариантность проверяется тестом: тот же рынок, умноженный на
    степень десяти, обязан дать ту же разметку.
    """
    # Нули и отрицательные цены в данных быть не должны, но log10 от них даёт
    # -inf и уронил бы разметку целиком — прогон упал бы на одном плохом баре.
    safe = close.where(close > 0)
    step = np.power(10.0, np.floor(np.log10(safe)) - 1)
    remainder = safe % step
    near = (remainder < ROUND_LEVEL_TOLERANCE * step) | (
        remainder > (1.0 - ROUND_LEVEL_TOLERANCE) * step
    )
...
def detect_atoms(base: pd.DataFrame) -> pd.DataFrame:
    ...
    a["round_level_touch"] = round_level_touch(close)
...
def _attach_sources(a: pd.DataFrame, base: pd.DataFrame) -> None:
    ...
    Пустые колонки в блок не попадают (все они контекстные) и в
    `bar_events.context_atoms` не пишутся — там хранятся только активные имена.

    Обход реестра вместо цепочки `if config.<источник>.enabled`: расчёт
    подключается объявлением источника, а не правкой этой функции. Что реестр
    при этом НЕ определяет — порядок атомов: имена берутся из `ATOM_FAMILY`,
    и биты остаются на своих местах при любом составе источников.

    Импорт внутри функции: расчёт источника тяжёлый, а `detect_atoms` зовут
    и из мест, где все источники выключены.
    """
    from btcproc.features import registry
