"""
Гейт N (docs/task_fear_greed.md §1.2) и `fgi_residual` как общая утилита.

Живёт отдельно от `features/fear_greed.py` намеренно — см. докстринг того
модуля: `FeatureSource.compute(base)` получает только сырые бары, а остатку
нужен уже посчитанный вектор из 32 признаков. Это МЕРА, а не продакшен-канал:
её считают измерительные скрипты (`fgi_frequencies.py`, `measure_fgi_range.py`,
`measure_feature_gradation.py`), у которых полный вектор уже есть через
`features/builder.py`.
...
на полной истории, протащат в остаток информацию из будущего (§1.2).
"""
