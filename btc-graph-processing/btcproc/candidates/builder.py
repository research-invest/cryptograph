...
def build_snapshots(
    ...
    # Офсеты отсчитываются ПОЗИЦИОННО (`+ shift_bars`), а не по настенным
    # часам. На дыре в барах «45 минут» окажется бо́льшим интервалом реального
    # времени. Это известное свойство, а не дефект (аудит 2026-08-15, B6):
    # фильтр `same_state` ниже и `valid`-логика исходов отсекают снимки,
    # разъехавшиеся с конфигурацией, поэтому дыра даёт не кривой снимок, а
    # отсутствующий. Дыры при этом стали реже — бар с нулевым диапазоном
    # больше не выпадает из признаков (B8).
