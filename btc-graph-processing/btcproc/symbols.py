...
SYMBOLS: tuple[SymbolSpec, ...] = (
    ...
    SymbolSpec(
        "AAVEUSDT", "2020-10-01",
        note=(
            "Листинг 2020-10-15 (проверено resolve_history_start) — история "
            "почти как у SOLUSDT, на два месяца короче. Спот Binance, перп на "
            "USD-M есть (метрики забэкфилены с общей границы архива). "
            "DeFi-протокол: ликвидность заметно ниже трёх старших монет, "
            "профиль оценки в btc-graph — по пресету low-liquidity. "
            "Состояний на первом train (#38) — 24 при 204 тыс. баров, то есть "
            "вдвое меньше, чем у SOLUSDT на сопоставимой истории. Это НЕ "
            "порог размера: минимальная группа 572 при эффективном пороге 508, "
            "дробление остановил gap-критерий. Диапазон штатный (у ETHUSDT "
            "под тем же split_gain_sigma=2.0 бывает 24–35), доля "
            "transition+event_block здоровая (15.6%), поэтому states_overrides "
            "намеренно не заданы"
        ),
        # metrics_start="" → METRICS_ARCHIVE_START (2021-12-01): перп AAVE
        # листился раньше, но архив метрик начинается общей границей.
    ),
    SymbolSpec(
        "TAOUSDT", "2024-04-01",
        note=(
            "Bittensor. Листинг на споте Binance 2024-04 (проверено "
            "resolve_history_start) — истории около двух с половиной лет, "
            "втрое короче, чем у SOLUSDT и AAVEUSDT, но вдвое длиннее, чем "
            "у HYPEUSDT. Осторожно с тикером: TAOUSDT (Bittensor) — не "
            "TAUUSDT и не WTAOUSDT. Перп на USD-M есть, архив метрик "
            "начинается 2024-04-11 (бисекция: 2024-04-10 → 404), то есть "
            "ПОЗЖЕ общей границы архива, и общая граница дала бы 2.5 года "
            "холостых 404 на каждом бэкфилле. "
            "min_group_size=550 — первый случай, когда порог пришлось поднять, "
            "а не опустить: относительный порог дал 0.0025 × 81 тыс. = 202, "
            "то есть монета упёрлась в абсолютный пол 300 — вдвое ниже "
            "эффективных порогов SOLUSDT (521) и AAVEUSDT (508), у которых "
            "истории втрое больше. Граф на полу дробился до 62 состояний "
            "(вне нормы 25–50) и разваливал выборку на переход: 998 рёбер, "
            "2360 кандидатов и НОЛЬ scope=transition+event_block. При 550 "
            "(замер: run 40 против run 41 на тех же барах) — 37 состояний, "
            "514 рёбер, 4987 кандидатов и 140 полных пар. Доля "
            "transition+event_block всё равно низкая (2.8% против 15.6% у "
            "AAVEUSDT) — это цена истории в два с половиной года, а не порога"
        ),
        states_overrides={"min_group_size": 550},
        metrics_start="2024-04-11",
    ),
    SymbolSpec(
        "PUMPUSDT", "2025-09-01",
        note=(
            "Pump.fun. Спот Binance с 2025-09-11 (первый дневной дамп; "
            "проверено листингом S3-бакета data.binance.vision) — САМАЯ "
            "КОРОТКАЯ история в реестре, около года, короче даже HYPEUSDT. "
            "Следствие то же, что у HYPEUSDT: выборка кандидата набирается из "
            "снимков конфигураций, и до min_sample_size=30 аналогов на пару "
            "(переход × блок событий) на такой истории почти ничего не "
            "добирается — доля scope=transition+event_block близка к нулю по "
            "длине истории, а не по поломке. Профиль оценки в btc-graph — по "
            "пресету young. "
            "Перп на USD-M листился ЗАДОЛГО до спота: архив метрик начинается "
            "2025-04-12 (первый файл неполный, 114 строк с 14:35 UTC), то есть "
            "на пять месяцев раньше первого бара. Деривативный контекст на "
            "барах доступен с самого начала истории, холостыми остаются только "
            "апрель–август в самой таблице deriv_metrics"
        ),
        metrics_start="2025-04-12",
    ),
)
