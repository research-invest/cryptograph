...
## 26. 🟡 Ребро Neo4j: `avg_horizon_return` хранил win rate — 2026-08-09

Из аудита `crypto-graph/docs/audit_2026-08-09.md`, находка M1. Свойство ребра
`TRANSITION` называлось «средний исход на горизонте», а записывался в него
`evaluation.win_rate` — и при создании ребра, и в скользящем среднем
(`graph_repo._upsert_tx`). Читалось оно там же как `avg_return`
(`_find_transitions_tx`). Любой Cypher-запрос «какой средний return у
перехода» получал не ту величину, причём молча: обе величины лежат в [0, 1]
и выглядят одинаково правдоподобно.

### Решение владельца (2026-08-09): переименовать

Из трёх вариантов — переименовать, начать писать настоящий horizon return,
оставить и задокументировать — выбран **первый**. Обоснование: числа в
свойстве корректны сами по себе, неверно только имя, а замена значения
сделала бы накопленное скользящее среднее смесью двух разных величин
(пересчёта записей нет).

Что сделано:

* свойство переименовано в `avg_win_rate` в `_upsert_tx` (обе ветви,
  `ON CREATE` и `ON MATCH`);
* чтение в `_find_transitions_tx` отдаёт поле как `avg_win_rate`, а не как
  `avg_return` — прежний алиас закреплял ту же ошибку в ответе API;
* существующие рёбра переводит `scripts/migrate_graph_avg_win_rate.cypher`
  (идемпотентный, с проверкой в конце). Прогнан на локальном графе: 615 рёбер,
  `stale = 0`, повторный запуск ничего не меняет;
* README и docstring модуля приведены в соответствие.

**Настоящего среднего исхода на ребре по-прежнему нет.** Если он понадобится —
это новое свойство и новое значение из `evaluation`, а не переиспользование
этого. Формула инкрементального среднего при этом верна и не трогалась —
см. отозванное замечание №5.
