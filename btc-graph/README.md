...
13. [Известные проблемы кода](#13-известные-проблемы-кода)
...
| **directional** | 35% | win rate, \|`outcome_skew`\|, F/A ratio | > 0.70, > 0.40, > 4.0 |
...
3. в каждой `candidate_family_key` остаётся один — **сначала предпочитается `fresh` над `stale`, и только потом сравнивается score** (см. [замечание №9 в аудите](docs/audit_findings.md));
...
| Метод | Путь | Описание |
|---|---|---|
| `POST` | `/queue/enqueue` | Положить кандидата в Redis Stream |
...
- **`favorable_adverse_ratio` берётся из long-метрик как есть**, без инверсии для short-кандидатов — для short его следует интерпретировать осторожно ([замечание №8 в аудите](docs/audit_findings.md)).
...
| Оценка вернулась мгновенно и с чужим `candidate_id` | Сработала дедупликация по `configuration_hash` в Redis. Обнови хэш, подожди 30 мин или отправь с `save=false` |
...
193 теста на чистую логику: парсер, валидатор, скорер, фильтры, pipeline.
...
| `tests/test_pipeline.py` | Детерминированная оценка, батч, деградация `_persist` при сбоях хранилищ |
...
|---|---|---|
| 1 | В `_score_context` недостижимая ветка: `age_60_120` оценивался как `age_lt_30` | Заменено на карту `_AGE_SCORE` с градацией 1.0 / 0.75 / 0.5 / 0.0 |
| 2 | ORM-объекты читались после закрытия сессии → 500 на трёх эндпоинтах | `sessionmaker(..., expire_on_commit=False)` |
| 3 | `refresh_continuous_aggregates` выполнялся внутри транзакции | Вызов вынесен на соединение с `isolation_level="AUTOCOMMIT"` |
| 4 | `_persist()` глушил все исключения `except: pass` | Логирование сбоев + возврат статуса по каждому хранилищу; то же в `graph_repo` и `redis_cache` |

**Важно:** правка №1 меняет оценки. Кандидаты с `age_30_60` теперь получают
0.75 вместо 1.0, с `age_60_120` — 0.5 вместо 1.0, то есть их `quality_score`
снижается до 0.05 и 0.10 соответственно. Оценки, сохранённые до этой правки,
несравнимы с новыми.

### Осталось (см. [`docs/audit_findings.md`](docs/audit_findings.md))

| № | Проблема | Эффект |
|---|---|---|
| 5 | Инкрементальное среднее в Cypher использует уже увеличенный `count` | `avg_horizon_return` и `avg_quality_score` в графе смещены |
| 6 | Дедупликация возвращает оценку другого кандидата | Ответ приходит с чужим `candidate_id` |
| 7 | `load_dotenv()` не вызывается | Локальный запуск не читает `.env` |
| 8 | Для short-кандидатов не инвертируется F/A ratio | Ось `directional` (вес 35%) начисляет баллы за long-метрику |
| 9 | `select_best_per_family`: свежесть важнее качества абсолютно | Слабый `fresh` вытесняет сильный `stale` |
| 10 | Пороги рейтинга продублированы в 4 местах | Риск рассинхронизации `/score/quick` и `/evaluate/json` |
| 11 | Вызов Claude без обработки ошибок и таймаута | Сбой API → 422 вместо 500 |
