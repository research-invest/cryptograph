...
Приёмник и вторая половина контура [crypto-graph](../README.md): принимает снимок текущей конфигурации рынка с историческими метриками и отвечает на вопрос **«насколько этой исторической аналогии можно доверять и в какую сторону она смещена»**. Кандидатов поставляет генератор из соседнего каталога — [`btc-graph-processing/`](../btc-graph-processing/README.md).
...
**`group_id` осмыслен только в паре с МОДЕЛЬЮ, а не только с монетой.** Ключ
узла — `(symbol, group_id)`, и измерения модели в нём нет. Между тем генератор
перенумеровывает состояния при каждом полном обучении: «группа 7» новой модели
— другой рынок, чем «группа 7» прежней. Без вмешательства накопленные `count`,
`avg_win_rate` и `avg_quality_score` продолжали бы расти поверх узлов
предыдущего поколения, и отличить одно от другого в графе невозможно: те же
свойства, те же диапазоны, правдоподобные `group_id`.

Поэтому генератор **сносит граф монеты** (`graph_repo.clear_symbol`) сразу
после того, как сохранил новую модель состояний. Практические следствия:

* граф всегда содержит ровно одно поколение нумерации — то, которым размечает
  текущая модель. Сравнивать рёбра между собой корректно;
* накопленная статистика рёбер обнуляется при каждом переобучении генератора
  (штатно — раз в неделю) и набирается заново из `live`. Это принятая плата:
  система в фазе становления, и пересборка дешевле усложнения контракта;
* если понадобится сохранять поколения — в ключ узла добавляется `model_id`,
  и его придётся протащить из генератора через схему кандидата. Разбор
  компромисса — в докстринге `clear_symbol`.
...
Ручки делятся на две части с разным режимом доступа. **`/api/v1/*` — публичное
API чтения, закрытое ключом** из `API_KEYS`; всё остальное аутентификации не
имеет и рассчитано на то, что порт 8000 слушает только петлю. Смешивать их
нельзя: открытая ручка, заведённая рядом с закрытыми, — самый простой способ
отдать наружу то, что закрывали.

### Публичное API чтения (ключ обязателен)

| Метод | Путь | Описание |
|---|---|---|
| `GET` | `/api/v1/ping` | Проверка ключа. В базу не ходит — отвечает и при лежащем PostgreSQL |
| `GET` | `/api/v1/symbols` | Монеты с сохранёнными оценками и метка калибровки каждой |
| `GET` | `/api/v1/candidates?symbol=BTCUSDT` | **Основная ручка**: кандидаты монеты с их оценками |
| `GET` | `/api/v1/candidates/{symbol}/{id}?include_raw=false` | Один кандидат; `include_raw=true` приложит досье генератора |

Ключ передаётся заголовком `X-API-Key: <ключ>` либо `Authorization: Bearer <ключ>`.
`401` — ключа нет или он неизвестен, `403` не используется. **`503` означает,
что `API_KEYS` пуст**, то есть API не настроено; пустое значение никогда не
трактуется как «пускать всех».

Фильтры `/api/v1/candidates`: `rating` (`STRONG` / `MODERATE` / `WEAK`, можно
через запятую), `direction` (`long` / `short`), `min_score`, `hours` (только
оценки за последние N часов), `order` (`recent` — по времени, `score` — по
качеству), `limit` (≤ 200) и `offset`. `total` в ответе считается по фильтру
целиком, а не по странице.

`symbol` обязателен, а смешанная выдача пишется явным `symbol=all` — и тогда
сортировка `order=score` идёт по `quality_score_baseline`: профильные
`quality_score` разных монет не сравнимы, и «топ» по ним означал бы «монета с
самой щедрой калибровкой».
...
curl -H "X-API-Key: $KEY" \
  'http://localhost:8000/api/v1/candidates?symbol=BTCUSDT&rating=STRONG&direction=long&hours=24'

# лучшее по всем монетам — сравнение идёт по baseline
curl -H "X-API-Key: $KEY" \
  'http://localhost:8000/api/v1/candidates?symbol=all&order=score&limit=10'
```

Числа отдаются **как их записал скорер в момент оценки**, вместе с меткой
калибровки (`scoring_profile` + `profile_fingerprint`); пересчёта по текущему
профилю нет — иначе клиент получил бы число, которого в истории не было.
Описание для чужого сервиса (можно отдать как есть) — `docs/api_spec_v1.md`;
устройство и выкладка наружу — `docs/public_api.md`.
...
| `ENABLE_CONFIG_RELOAD` | `false` | Разрешить `POST /config/reload`. Ручка меняет поведение скоринга, а внутренние ручки аутентификации не имеют — включать только локально |
| `API_KEYS` | — | Ключи публичного API чтения (`/api/v1/*`): через запятую, элемент — `метка:ключ` либо голый ключ. **Пусто = 503 на `/api/v1/*`**, а не открытый доступ. Читается на каждом запросе: смена ключа требует `make reload`, но не пересборки |
