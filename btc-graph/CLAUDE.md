...
- `docs/public_api.md` — публичное API чтения `/api/v1/*`: ключ, устройство, выкладка через nginx.
- `docs/api_spec_v1.md` — **внешний контракт**: текст, который отдаётся чужому сервису или
  кладётся в промпт чужому агенту. Правишь ручки или модели ответа в `public.py` — проверь его,
  иначе описание и код разойдутся молча.
