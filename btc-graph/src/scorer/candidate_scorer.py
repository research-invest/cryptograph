...
def fa_ratio_for(c: Candidate) -> float | None:
    ...
    Long-версия описывает путь случаев роста: потенциал long и его риск.
    Short-версия — зеркало по случаям падения, где выгода и риск меняются
    местами. Раньше её не существовало в данных, short получал None и ось
    directional считалась ему по двум критериям вместо трёх; это давало
    систематический перекос не в пользу short (замер 2026-08-13: медиана оси
    0.100 против 0.400 у long).

    None остаётся возможным ответом и теперь — у кандидатов, выпущенных до
    появления поля, и у конфигураций, в выборке которых падений не было.
    Правило прежнее: критерий без данных выбывает, а не начисляет ноль.

    В профиль не выносится по той же причине, что и win_rate_for.
    """
    if c.research_side == ResearchSide.long:
        return c.long_favorable_adverse_ratio_p70_p80
    return c.short_favorable_adverse_ratio_p70_p80
...
def _score_directional(c: Candidate, profile: ScoringProfile | None = None) -> float:
    ...
    Усредняется по критериям, к которым у кандидата есть данные: обычно по
    трём, а если F/A ratio неизвестен — по двум (см. fa_ratio_for).

    Ступени общие на обе стороны. Отдельные ступени на сторону пробовались
    2026-08-13 и отвергнуты замером: после появления short-версии F/A ratio
    они не меняют ни медиану оси, ни её разброс (разбор — раздел 8 журнала).
    """
