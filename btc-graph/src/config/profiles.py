...
class StatisticalSpec(BaseModel):
    ...
    # Ступень по числу НЕЗАВИСИМЫХ реализаций перехода. Необязательна, и это
    # не мелочь, а способ провести правку без переоценки прошлого.
    #
    # `sample_size` считает строки снимков: генератор берёт снимок конфигурации
    # в момент перехода и спустя 45/90/180 минут. Отношение строк к случаям —
    # 1.5…2.8 (медиана 2.2), и оно не константа: оно зависит от того, как долго
    # держится состояние. То есть ранжирование по `sample_size` частично
    # ранжирует по живучести состояния, а не по объёму свидетельств.
    #
    # Профиль БЕЗ этой ступени считает ось как раньше — по `sample_size`.
    # Так остаётся неподвижным `_default`: baseline у всех кандидатов, старых и
    # новых, по-прежнему меряется одной линейкой (в этом вся его роль).
    # Профиль монеты ступень определяет, и тогда она применяется к кандидатам,
    # которые поле несут; у выпущенных до 2026-08-13 его нет, и они считаются
    # по `sample_size` — той ступенью, на которой и были выпущены.
    effective_sample_size: LadderSpec | None = None
