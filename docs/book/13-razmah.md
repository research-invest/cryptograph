...
## 13.9а. Оговорка, поставленная суррогатом (2026-08-24)

Самая важная правка этой главы, и она не отменяет вердикт, а меняет то, что
вердикт **значит**. Полный разбор — глава 12.11а, журнал генератора раздел 56;
здесь — следствия для размаха.

Прогон стенда на суррогатных данных, где предсказывать нечего в принципе, дал
$\Delta R^2 = +0.21$ — втрое больше, чем на реальных, — и прошёл все три
условия гейта. Причина оказалась в конструкции цели, а не в дефекте суррогата.

Цель в логарифмах распадается на разность:

$$\log(\text{range\_ratio}) = \log(\text{будущий размах}) - \log(\text{ATR}_t)$$

Вычитаемое известно **уже на баре $t$**. При нормировке `atr14` его окно — 14
баров, а самое короткое окно бенчмарка B2 — 96. Модель видит короткие
признаки, бенчмарк не видит ничего короче суток, и приращение $R^2$ частично
меряет **эту разницу**, а не способность предсказывать будущее.

Диагностический прогон (флаг `--augment-benchmark`, колонка
`rf.denominator_column`) отдаёт знаменатель бенчмарку. На суррогате приращение
падает с +0.21 до −0.007, то есть механизм объясняет его целиком. На реальных
данных:
...
Медиана — **46%**; все три условия выдерживают 3 ячейки из 8 против 7 из 8 у
штатного бенчмарка.

**Что это значит для продукта.** `range_lift` = p50 модели / p50 бенчмарка,
значит та же асимметрия сидит и в нём. Содержательно величина ближе к
«текущая волатильность необычна относительно своей суточной нормы», чем к
«рынок пойдёт шире обычного». Она по-прежнему полезна оператору — но называть
её прогнозом размаха преувеличение.

**Чего это НЕ значит.** Вердикт раздела 13.8 не отменяется: критерий был
заявлен против бенчмарка B2 и против него пройден. Условия не пересматриваются
по итогам — в том числе тогда, когда пересмотр был бы в сторону строгости.
Меняется описание, а не расчёт.

Незакрытый долг, записанный здесь же: пересчитать гейт с бенчмарком,
знающим знаменатель, и посмотреть, какие монеты проходят его тогда. Это меняет
состав монет, отдающих числа наружу, поэтому требует отдельного захода.

---

## 13.10. Что осталось незакрытым

**Прогноз показан пользователю — с 2026-08-24.** Долг закрыт: `range_lift` и
режим появились в таблице кандидатов, в общей сводке дашборда и в карточке
кандидата, где стоят ВЫШЕ блока оценки (оценка говорит о направлении, а про
направление измерено, что система его не предсказывает). Наружу идёт
относительная величина; абсолютные квантили — только в карточке и только с
оговоркой. Отсутствие полей подписано словами: кандидаты `train` их не несут
по построению, у монет без калиброванной модели их нет вовсе, и прочерк без
...
**Гейт не пересчитан с равноинформированным бенчмарком** — см. 13.9а.
...
И два урока про нормировку. Первый: когда предиктор и знаменатель цели
построены из одной величины, знак приращения принадлежит паре, а не предиктору.
Второй, добавленный 2026-08-24 (13.9а): **когда знаменатель цели известен на
баре $t$, бенчмарк обязан знать его тоже** — иначе приращение $R^2$ меряет
информационную асимметрию под видом предсказательной силы. Около половины
измеренного приращения оказалось именно этим.
