...
### Две команды, ходящие в чужой API

`fetch-external` (Fear & Greed) и `ingest-metrics` (деривативы Binance USD-M)
стоят **вне** `train`/`live` намеренно: сетевой поход не должен ронять или
замедлять регулярный расчёт. Прогоны только читают таблицы.
