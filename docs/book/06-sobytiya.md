...
Допуск 0.05 с каждой стороны даёт около 10% баров — столько же, сколько давала
прежняя формула, поэтому гранулярность блоков от смены детектора не поехала.
Замер на 8000 свежих баров BTC: 11.9%.

Масштабная инвариантность проверяется тестом: тот же рынок, умноженный на
степень десяти, обязан дать ту же разметку. (Проверено и вручную при подготовке
этой главы: `round_level_touch(close)` и `round_level_touch(close × 1000)`
совпадают на всех барах.)
