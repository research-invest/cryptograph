...
# проверка, что перенос ничего не порвал.
...
cat <<TXT
  ...
  Код на сервере обновлён, тесты прошли$( ((STACK_RUNNING)) && echo ", схема догнана" || echo " (стек не запущен — миграции пропущены)").
