...
# Снимки стакана. Единственное задание расписания, копящее данные РАНЬШЕ
# решения о том, зачем они нужны: историю стакана невозможно докупить, биржи
# отдают глубину только «на сейчас». Каждый пропущенный день вычитается из
# будущей выборки безвозвратно (ТЗ docs/tz_wave_a_24-08-26.md, задача E).
#
# Раз в минуту — это шесть коротких HTTP-запросов и одна вставка; на фоне
# live-прогона каждые полчаса нагрузка незаметна. Задание НИ НА ЧТО не
# влияет: ни train, ни live таблицу depth_snapshots не читают.
DEPTH_CRON="${DEPTH_CRON:-* * * * *}"
...
# Внутренние ручки (/evaluate/*, /queue/*, /stats/*) аутентификации не имеют
# и закрыты тем, что 8000 слушает только 127.0.0.1. Ручка перечитывания
...
# Ключ публичного API чтения (/api/v1/*). Дописывается отдельно от создания
# .env: на уже развёрнутой машине файл не перезаписывается (там правки
# оператора), а ключ ей нужен так же. Пустой API_KEYS означает «не настроено»
# и даёт 503, поэтому строку с заглушкой класть нельзя — только настоящий ключ.
if ! grep -qE '^API_KEYS=' btc-graph/.env; then
    printf 'API_KEYS=prod:%s\n' "\$(openssl rand -hex 32)" >> btc-graph/.env
    echo "  btc-graph/.env: сгенерирован API_KEYS (значение — в файле)"
fi
...
REMOTE_DIR="$REMOTE_DIR" LIVE_ARGS="$LIVE_ARGS" LIVE_CRON="$LIVE_CRON" \
    ...
    DERIV_CRON="$DERIV_CRON" DEPTH_CRON="$DEPTH_CRON" remote <<EOF
MARK="# crypto-graph"
...
\$MARK — снимки стакана в depth_snapshots, раз в минуту
\$MARK   Копится ВПРОК: историю стакана нельзя докупить ни за какие деньги,
\$MARK   биржи отдают глубину только «на сейчас». Решение «полезен ли он»
\$MARK   принимается примерно через год; решение «копить» — сегодня.
\$MARK   Ни train, ни live эту таблицу не читают, признаков из неё не заводится.
\$MARK   В лог пишется ТОЛЬКО сбой: минутное задание при построчном логе дало бы
